import React from 'react'

function AllTask() {
    return (
        <div className='bg-[#1c1c1c] p-5 rounded mt-3 h-40 overflow-auto'>
                <div className='bg-violet-400 py-2 px-4  mb-3 flex justify-between rounded'>
                    <h2>Ahmad</h2>
                    <h3>  make a ui design </h3>
                    <h5>status</h5>
                </div>
                <div className='bg-indigo-400 py-2 px-4 mb-3 flex justify-between rounded'>
                    <h2>Ahmad</h2>
                    <h3>  make a ui design </h3>
                    <h5>status</h5>
                </div>
                <div className='bg-blue-400 py-2 px-4  mb-3 flex justify-between rounded'>
                    <h2>Ahmad</h2>
                    <h3>  make a ui design </h3>
                    <h5>status</h5>
                </div>
                <div className='bg-green-400 py-2 px-4 mb-3 flex justify-between rounded'>
                    <h2>Ahmad</h2>
                    <h3>  make a ui design </h3>
                    <h5>status</h5>
                </div>
                <div className='bg-yellow-400 py-2 px-4 mb-3 flex justify-between rounded'>
                    <h2>Ahmad</h2>
                    <h3>  make a ui design </h3>
                    <h5>status</h5>
                </div>
                <div className='bg-orange-400 py-2 px-4 mb-3 flex justify-between rounded'>
                    <h2>Ahmad</h2>
                    <h3>  make a ui design </h3>
                    <h5>status</h5>
                </div>
                <div className='bg-red-400 py-2 px-4 mb-3 flex justify-between rounded'>
                    <h2>Ahmad</h2>
                    <h3>  make a ui design </h3>
                    <h5>status</h5>
                </div>
        </div>
    )
}

export default AllTask
